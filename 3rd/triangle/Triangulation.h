...
public:
    inline stack() : m_data() {}
    inline stack(const size_t size) : m_data(size) {}
    inline stack(const value_t* start, const value_t* end) : m_data(start, end) {}
    inline const value_t& top() const { return m_data[m_data.size() - 1]; }
    inline void pop() { m_data.pop_back(); }
    inline void push(const value_t& value) { m_data.push_back(value); }
    template<class... args_t> inline void emplace(args_t&&... args) { m_data.emplace_back(args...); }
    inline void reserve(const size_t capacity) { m_data.reserve(capacity); }
    inline void resize(const size_t size) { m_data.resize(size); }
    inline size_t size() const { return m_data.size(); }
    inline bool empty() const { return m_data.empty(); }
    inline value_t& operator[](const size_t index) { return m_data[index]; }
    inline const value_t& operator[](const size_t index) const { return m_data[index]; }
...
private:
    ...
    void insertVertexInsideTriangle(VertInd v, TriInd iT, stack<TriInd>& out);
    /// Returns indices of four resulting triangles
    void insertVertexOnEdge(VertInd v, TriInd iT1, TriInd iT2, stack<TriInd>& out);
    ...
    TriIndUSet growToBoundary(stack<TriInd> seeds) const;
