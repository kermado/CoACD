...
/* Flag if triangulation is success                                          */
...
/*  triangles are four-byte-aligned and that the `int64_t' type is     */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((int64_t) (ptr) & (int64_t) 3l);         \
  (otri).tri = (triangle *)                                                   \
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((int64_t) (sptr) & (int64_t) 1l);      \
  (osub).ss = (subseg *)                                                      \
...
#ifdef ANSI_DECLARATORS
int64_t randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  while (leftflag) {
    ...
    apex(*searchtri, leftvertex);
    rightccw = leftccw;
    leftccw = counterclockwise(m, b, searchpoint, startvertex, leftvertex);   // xy: it may have infinite loop
    leftflag = leftccw > 0.0;
    if(count ++ > 1e6)
    {
      b->is_success = false;
      cout << "manually designed stop" << endl;
      return WITHIN;
    }
  }
  count = 0;
...

{
  ...
  if (b->poly) {
    ...
    m->insegments = (int)segments.size();
#endif /* not TRILIBRARY */
...
#ifdef ANSI_DECLARATORS
void readnodes(struct mesh *m, struct behavior *b, vector<array<double, 2>> vertices)
#else /* not ANSI_DECLARATORS */
void readnodes(m, b, vertices)
struct mesh *m;
struct behavior *b;
...

{
  ...
  m->invertices = (int)vertices.size();
  m->mesh_dim = 2;
  m->nextras = 0;
  if (m->invertices < 3) {
  ...
  for (i = 0; i < (int)vertices.size(); i++) {
    ...
    if (i == 0) {
      firstnode = (int)vertices.size();
      if ((firstnode == 0) || (firstnode == 1)) {
        b->firstnumber = firstnode;
      }
    }

    vertexloop[0] = vertices[i][0];
    vertexloop[1] = vertices[i][1];
...
#else /* not ANSI_DECLARATORS */
void readholes(m, b, hlist, holes, rlist, regions)
struct mesh *m;
struct behavior *b;
REAL **hlist;
int *holes;
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh *m, struct behavior *b, vector<array<double, 2>>& nodes)
#else /* not ANSI_DECLARATORS */
void writenodes(m, b, nodes)
struct mesh *m;
struct behavior *b;
  ...
  while (vertexloop != (vertex) NULL) {
    if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
      ...
      // fprintf(outfile, "%4d    %.17g  %.17g", vertexnumber, vertexloop[0],
              // vertexloop[1]);
      nodes.push_back({vertexloop[0], vertexloop[1]});
      for (i = 0; i < m->nextras; i++) {
        /* Write an attribute. */
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    if (b->order == 1) {
      ...
      triangles.push_back({vertexmark(p1), vertexmark(p2), vertexmark(p3)});
#endif /* not TRILIBRARY */
    } else {
...

{
  ...
  struct behavior b;
#ifndef TRILIBRARY
  // vector<array<double, 2>> vertices = {{0.0, 0.0}, {1.0, 0.0}, {1.5, 0.5}, {1.0, 1.0}, {0.0, 1.0}};
  // vector<pair<int, int>> segments = {{1,2}, {2,3}, {3,4}, {4,5}, {5,1}};
  // vector<array<int, 3>> triangles;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  {
    int anum = 3;
    char * a[] ={(char*)"./triangle", (char*)"-p", (char*)"A.poly"};
    char **acom = a;
    parsecommandline(anum, acom, &b);
  }
  else
  {
    int anum = 4;
    char * a[] ={(char*)"./triangle", (char*)"-u", (char*)"-D", (char*)"A.node"};
    char **acom = a;
    parsecommandline(anum, acom, &b);
  }
  ...
  if (b.usesegments) {
    ...
    if (!b.refine) {
      ...
      if(!b.is_success)
      {
        is_success = b.is_success;
        return 0;
      }
        
#else /* not TRILIBRARY */
      formskeleton(&m, &b, segments);
      if(!b.is_success)
      {
        is_success = b.is_success;
        return 0;
...
#else /* not TRILIBRARY */
    // readholes(&m, &b, &holearray, &m.holes, &regionarray, &m.regions);
#endif /* not TRILIBRARY */
    if (!b.refine) {
